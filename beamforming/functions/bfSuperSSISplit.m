function [IQOut,BFConfig] = bfSuperSSISPlit(RcvData,varargin)
%% This function is a lazy function to beamform SuperWide SSI/NSSI data
% Created by Baptiste Heiles on 07/03/2022
% IQOut=bfVerasonics(RcvData) beamforms the RcvData with an OPW kernel and
% a fNumber=1
% IQOut=bfSuperSSI(RcvData,fNumber); beamforms the RcvData with an OPW
% kernel and a custom fNumber
% IQOut=bfSuperSSI(RcvData,fNumber,kernel_name); beamforms the RcvData
% with a custom fNumber and kernel


%% Create BFConfig structure with the Verasonics mode
BFConfig=makeBFConfig('SuperWideNSSI');
...
try
    ...
    RF=arrangeRF2SSI([RcvData(:,:,1);RcvData(:,:,2)],BFConfig.Nz_RF,BFConfig.NAnglesPerBis*BFConfig.NBisectorsTot/2,modeSSI);
catch
    disp('RF to NSSI');
    RF=arrangeRF2SSI([RcvData(:,:,1);RcvData(:,:,2)],BFConfig.Nz_RF,BFConfig.NAnglesPerBis*BFConfig.NBisectorsTot/2,'NSSI');
end
